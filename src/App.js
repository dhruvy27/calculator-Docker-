...
//only keycodes that the user is allowed to press on keyboard
const usedKeyCodes = [
...
function App() {
  ...
  const handleKeyPress = (keyCode, key) => {
    ...
    if (numbers.includes(key)) {
      ...
      //if last character is operator then exclude
      const lastChar = expression.slice(-1);
      if (operators.includes(lastChar)) return;
      if (lastChar === ".") return;

      //if last character is decimal then exclude
      setExression(expression + key);
    } else if (key === ".") {
      if (!expression) return;
      const lastChar = expression.slice(-1);
      ...
      //handling backspace
      //if expression is empty dont work
      setExression(expression + key);
    } else if (keyCode === 8) {
      if (!expression) return;
  ...
  const calculateResult = (exp) => {
    ...
    //if last char is not a number then trim
